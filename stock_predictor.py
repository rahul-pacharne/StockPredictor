...
class Stocker:
    def __init__(self, ticker):
        ...
        stock1 = hist.reset_index(level=0)

        # Columns required for prophet
        stock1["ds"] = stock1["Date"]
        if "Adj. Close" not in stock1.columns:
            stock1["Adj. Close"] = stock1["Close"]
            stock1["Adj. Open"] = stock1["Open"]

        stock1["y"] = stock1["Adj. Close"]
        stock1["Daily Change"] = stock1["Adj. Close"] - stock1["Adj. Open"]
        self.stock_h = stock1
        self.min_date = min(stock1["Date"])
        self.max_date = max(stock1["Date"])
    ...
    def plot_stock(
        ...
        for i, stat in enumerate(stats):
            ...
            print(
                "Current {} = {:.2f} on {}.\n".format(
                    stat, self.stock_h.loc[self.stock_h.index[-1], stat], self.max_date
                )
            )
    ...
    def buy_and_hold(self, start_date=None, end_date=None, nshares=1):
        ...
        start_price = float(self.stock_h[self.stock_h["Date"] == start_date]["Adj. Open"])
        end_price = float(self.stock_h[self.stock_h["Date"] == end_date]["Adj. Close"])
    ...
    def changepoint_prior_analysis(
        ...
        train = self.stock_h[
            (
                self.stock_h["Date"]
                > (max(self.stock_h["Date"]) - pd.DateOffset(years=self.training_years))
            )
        ]
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares=None):
        ...
        test = self.stock_h[
            (self.stock_h["Date"] >= start_date) & (self.stock_h["Date"] <= end_date)
        ]
    ...
    def predict_future(self, days=30):
        ...
        future = future[future["ds"] >= max(self.stock_h["Date"])]
